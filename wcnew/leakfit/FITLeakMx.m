...
x = (t-T(1)).*(t>=T(1)).*(t<=T(2));
P2 = Params(2)*ones(size(t)) +...
     Params(3)*exp(-x/Params(4)) + ...
     Params(5)*exp(-x/Params(6));

x = (t-T(2)).*(t>=T(2)).*(t<=T(3));
P3 = Params(7)*ones(size(t)) + ...
     Params(8)*exp(-x/Params(9)) + ...
     Params(10)*exp(-x/Params(11));
 
x = (t-T(3)).*(t>=T(3)).*(t<=T(4));
P4 = Params(12)*ones(size(t)) + Params(13)*exp(-x/Params(14)) + Params(15)*exp(-x/Params(16));

myleakfit =Params(1)+ P2.*(t>=T(1)).*(t<=T(2))+...
    +P3.*(t>=T(2)).*(t<=T(3))+ P4.*(t>=T(3)).*(t<=T(4));
% myleakfit = MovingAvg(Params(17),5,myleakfit);
